...
export default function SearchResult({ searchValue, data }) {
	return (
		<div className="flex fixed text-black py-4 w-full justify-center">
			<div className="w-3/4 bg-white ml-5 sm:w-2/4 flex">
				<div className="flex flex-col w-full">
					{searchValue !== "" &&
						data !== null &&
						data // eslint-disable-next-line
							.filter((data) => {
								if (data.slug.current.toLowerCase().includes(searchValue.toLowerCase()))
									return data;
							})
							.map((data, index) => (
								<a
